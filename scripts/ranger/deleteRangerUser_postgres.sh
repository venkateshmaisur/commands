#!/bin/bash
## Script to delete Ranger Users from Postgres database
## Usage: deleteRangerUser_postgres.sh -f input.txt -u ranger_user -p password -db ranger [-r <replaceUser>]
...
do
    ...
    $mysqlex -c "SELECT deleteUserByUsername('$name', '$superuser')"
done < $filename

$mysqlex -c "DROP FUNCTION IF EXISTS deleteUserByUsername(username varchar(1024), superuser varchar(1024))"
